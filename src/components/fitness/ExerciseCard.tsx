    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-1">
          {hasMedia && <ImageIcon className="h-3 w-3 text-brand-electric" />}
          <DifficultyBadge level={difficulty} />
        </div>
      </div>
      <p className="mt-1 text-[11px] text-white/40">{useful}</p>
...
function DifficultyBadge({ level }: { level: "Beginner" | "Intermediate" | "Advanced" }) {
  const cls = level === "Beginner" ? "text-green-400 bg-green-400/10" : level === "Advanced" ? "text-orange-400 bg-orange-400/10" : "text-blue-400 bg-blue-400/10";
  return <span className={`rounded px-1 py-0.5 text-[9px] font-medium ${cls}`}>{level}</span>;
}

function Tag({ children, color }: { children: React.ReactNode; color: "violet" | "blue" | "default" }) {
  const cls = color === "violet" ? "bg-violet-500/10 text-violet-300" : color === "blue" ? "bg-blue-500/10 text-blue-300" : "bg-white/10 text-white/60";
